...
void EventLoop::Loop(){
   ...
   // contain the higgs though numerous methods that identify the expected value against the value seen. 
      ...
      if(jentry%10000 == 0)std::cout<<"Processing "<<jentry<<" events!!!"<<std::endl;
      for(auto sel : mySel){
          pass_sel[sel] = false;
      }
      m_NTags        = 0;
      ...
      if(!passed_merged_preselection && !passed_resovled_preselction)continue;
      ...
      if(passed_merged_preselection){
         if(Lepton_Charge > 0){
            if((Higgs.M()*0.001  > hm_lower_bound && Higgs.M()*0.001 < hm_upper_bound))pass_sel["Merged_LepP_SR"]  = true;
            if(!(Higgs.M()*0.001 > hm_lower_bound  && Higgs.M()*0.001 < hm_upper_bound))pass_sel["Merged_LepP_CR"]  = true;
         }else if(Lepton_Charge < 0){
            if(((Higgs.M()*0.001  > hm_lower_bound && Higgs.M()*0.001 < hm_upper_bound)  && (Wplus.M()*0.001 > wm_lower_bound && Wplus.M()*0.001 < wm_upper_bound)))pass_sel["Merged_LepN_SR"] = true;
            if(!((Higgs.M()*0.001 > hm_lower_bound && Higgs.M()*0.001 < hm_upper_bound)  && (Wplus.M()*0.001 > wm_lower_bound && Wplus.M()*0.001 < wm_upper_bound)))pass_sel["Merged_LepN_CR"] = true;
         }
      }
      ...
      if(passed_resovled_preselction && !pass_sel["Merged_LepP_SR"] && !pass_sel["Merged_LepN_SR"]){
         if(Lepton_Charge > 0){
            if(m_MaxMVA_Response > 0.7){
                  ...
                  if(pass_sel["Merged_LepP_CR"] == true)pass_sel["Merged_LepP_CR"] = false;
            }
            if(!pass_sel["Merged_LepP_CR"] && m_MaxMVA_Response > -0.5 && m_MaxMVA_Response < 0.3)pass_sel["Resolved_LepP_CR"] = true;
         }else if(Lepton_Charge < 0){
            if(m_MaxMVA_Response > -2.0){
                  pass_sel["Resolved_LepN_SR"]  = true;
                  if(pass_sel["Merged_LepN_CR"] == true)pass_sel["Merged_LepN_CR"] = false;
            }
       	    if(!pass_sel["Merged_LepN_CR"] && m_MaxMVA_Response < 0.5)pass_sel["Resolved_LepN_CR"] = true;
         }
      }
...
bool EventLoop::PassEventSelectionBoosted(){
    ...
    if(Lepton_Charge < 0){
       ...
       if(FJets.size() < 1)return false;
       if(FJets.at(0).Pt() < 250000)return false;
       if(!FindFJetPair())return false;
    }
    return true;
}
...
bool EventLoop::PassEventSelectionResolved(){    
    ...
    if(Lepton_Charge < 0){
        ...
        if(m_NTags_caloJ < 2)return false;
        if(Jets.size()   < 4)return false;
       	if(FindJetPair_lvbb() == false)return false;
        if(Higgs.Pt()*0.001 < 100.)return false;
        if(Wplus.Pt()*0.001 < 120.)return false;
        return true;
    }
    return true;
}
