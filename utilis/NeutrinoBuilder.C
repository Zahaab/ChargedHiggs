...
{
  if (m_debug > 0)
    std::cout << "entering NeutrinoBuilder::candidatesFromWMass_Scaling()" << std::endl;
...
bool NeutrinoBuilder::candidatesFromWMass_Scaling(const TLorentzVector *L, const double met_etx, const double met_ety, std::vector<TLorentzVector *> &NC)
{
  ...
  do
  {
    ...
    if (m_debug > 1)
      std::cout << "count = " << count << "\tptNu = " << ptNu << "\tdelta = " << delta << "\tm_mWpdg = " << m_mWpdg << std::endl;
  ...
  {
    if (m_debug > 0)
      std::cout << "no solution delta still<0" << std::endl;
    return false;
  }
  delta = sqrt(delta);
  ...
  NC.push_back(nu2);
  if (m_debug > 0)
    std::cout << "quitting NeutrinoBuilder::candidatesFromWMass_Scaling()" << std::endl;
  return true;
}
